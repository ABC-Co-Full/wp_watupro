...
function watupro_cats() {	
	...
	switch(@$_GET['do']) {
		...
		default:
			// select my cats
			$own_sql = ($multiuser_access == 'own') ? $wpdb->prepare(" AND C.editor_id = %d ", $user_ID) : "";
			$cats = $wpdb->get_results($wpdb->prepare("SELECT C.*, COUNT(Q.ID) as num_quizzes 
                FROM ".WATUPRO_CATS." C
                LEFT JOIN ".WATUPRO_EXAMS." Q ON Q.cat_id = C.ID
                WHERE C.parent_id=%d $own_sql 
                GROUP BY C.ID
                ORDER BY C.name", $parent_id));

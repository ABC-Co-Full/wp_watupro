...
$result = $unresolved_questions = $resolved_questions = $incorrect_questions = $unanswered_questions = $current_text = $paginated_result = $short_answers = '';
...
foreach ($all_question as $qct => $ques) {	
      ...
      $all_feedbacks[] = $answer_feedback;
	   ...
	   if($is_empty) $unanswered_questions .= $current_text;
      ...
      if(isset($unresolved_by_cat[$ques->cat_id])) $unresolved_by_cat[$ques->cat_id][] = $unresolved_text;
        ...
        if(!$ques->exclude_on_final_screen and !in_array($ques->cat_id, $question_catids_closed) and ($exam->single_page == WATUPRO_PAGINATE_PAGE_PER_CATEGORY or $exam->single_page == WATUPRO_PAGINATE_ONE_PER_PAGE) and $exam->group_by_cat) {
  		...
  		if(empty($ques->exclude_on_final_screen)) {
  			...
  			else $incorrect_questions .= $current_text;
  		}		 
...
'%%ANSWERS%%', '%%CATGRADES%%', '%%DATE%%', '%%EMAIL%%', '%%MAX-POINTS%%', '%%watupro-share-url%%',
	...
	'%%PERCENTOFMAXLEFT%%', '%%ANSWERS-NOFEEDBACK%%', '%%FEEDBACK-NOANSWERS%%', '%%ONLYFEEDBACK%%', '%%UNRESOLVED-MARKS%%', '%%UNANSWERED%%');
...
$with_these = array($score, $total,  $percent, $rating, $score, stripslashes($exam->name), wpautop(stripslashes($exam->description)), $achieved,  $certificate, 
   ...
   sprintf('%08d',$taking_id), $avg_percentofmax, $avg_percentofmax, $percent, round($pointspercent), $resolved_questions, $percent_wrong, 100 - round($pointspercent), $result_nofeedback, $feedback_noanswers, $onlyfeedback, $incorrect_questions, $unanswered_questions);

if(strstr($exam->final_screen, '%%TOTAL-ENTRIES%%') or strstr($exam->email_output, '%%TOTAL-ENTRIES%%')) {
	$replace_these[] = '%%TOTAL-ENTRIES%%';

	$total_entries = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".WATUPRO_TAKEN_EXAMS."
		WHERE exam_id=%d AND in_progress=0", $exam->ID));

		...
		foreach($grades as $grade) {
			...
			if(!$exists) {
				$wpdb->query($wpdb->prepare("INSERT INTO ".WATUPRO_GRADES." SET
					...
					redirect_url=%s, moola=%d, is_fallback=%d",
					$exam->ID, stripslashes($grade->gtitle), stripslashes($grade->gdescription), 
					$grade->gfrom, $grade->gto, $grade->certificate_id, $grade->cat_id, $grade->redirect_url, $grade->moola, $grade->is_fallback));
			}	
		}	
...
function watupro_grades() {
	...
	if(!empty($_POST['add']) or !empty($_POST['save'])) {
		...
		$_POST['certificate_id'] = intval($_POST['certificate_id'] ?? 0);
		$_POST['cat_id'] = intval($_POST['cat_id']);
		$_POST['redirect_url'] = esc_url_raw($_POST['redirect_url']);
		$_POST['moola'] = intval($_POST['moola'] ?? 0);
		$is_fallback = empty($_POST['is_fallback']) ? 0 : 1;
	}
	...
	if(!empty($_POST['add']) and check_admin_referer('watupro_grade')) {
		...
		$wpdb->query($wpdb->prepare("INSERT INTO ".WATUPRO_GRADES." SET
			...
			moola=%d, category_requirements=%s, is_fallback=%d",
			$exam->ID, $_POST['gtitle'], $_POST['gdescription'], $_POST['gfrom'], $_POST['gto'], intval($_POST['certificate_id'] ?? 0),
			$_POST['cat_id'], $_POST['redirect_url'], $_POST['moola'], $catreqs, $is_fallback));
	}
	...
	if(!empty($_POST['save']) and check_admin_referer('watupro_grade')) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_GRADES." SET
			gtitle=%s, gdescription=%s, gfrom=%s, gto=%s, certificate_id=%d, redirect_url=%s, moola=%d,
			category_requirements=%s, is_fallback=%d
			WHERE ID=%d",
			$_POST['gtitle'], watupro_strip_tags($_POST['gdescription'.$_POST['id']]), $_POST['gfrom'], $_POST['gto'], 
			intval($_POST['certificate_id'] ?? 0), $_POST['redirect_url'], $_POST['moola'], $catreqs, $is_fallback, $_POST['id']));
	}
	...
	// make sure there are no missing parents in $cats
	foreach($cats as $cat) {
		if($cat->parent_id and !in_array($cat->parent_id, array_column($cats, 'ID'))) {
			$parent = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".WATUPRO_QCATS." WHERE ID=%d", $cat->parent_id));
			$cats[] = $parent;
		}
	}
	
	// reorder subcats under their parents
	$parent_cats = array_filter($cats, function ($category) {
    ...
    foreach($parent_cats as $c) {
        $final_cats[] = $c;
        foreach($cats as $sub) {
           if($sub->parent_id == $c->ID) $final_cats[] = $sub;
        }
    } // end foreach

...
function watupro_view_certificate() {
	...
	if(empty($_GET['taking_id'])) {
		...
		$taken_date = date_i18n(get_option('date_format'), strtotime($taking->end_time));
	...
	if(get_option('watupro_generate_pdf_certificates') == "1") {
		...
		$cert_settings = $settings[$certificate->ID] ?? [];
		$file_name = "certificate-".$certificate->ID.'-'.intval($user_certificate->taking_id ?? 0).'.pdf';
		if(!empty($cert_settings['file_name'])) $file_name = $cert_settings['file_name'];
		$file_name = apply_filters('watupro_certificate_file_name', $file_name, $certificate->ID, $user_certificate->taking_id ?? 0);
...
function watupro_maybe_attach_certificate( $certificate_id, $taking_id ) {
    ...
    if(!empty($certificate_id) and $generate_pdf_certificates == "1" and $attach_certificates) {
        ...
        foreach($certificate_id as $cert_id) {
            ...
            if(@file_exists(WP_CONTENT_DIR . "/uploads/watupro/".$file_name)) {
                $file_name = preg_replace('/\.pdf$/', '', $file_name);
                $file_name = $file_name .'-'. substr(md5($_SERVER['REMOTE_ADDR']), 0, 6).'.pdf';
            }
            //die($file_name);

            $file_name = apply_filters('watupro_certificate_file_name', $file_name, $certificate_id, $taking_id);
            $_GET['download_file_name'] = $file_name;
            watupro_view_certificate();
            
            $attachments[] = WP_CONTENT_DIR . "/uploads/watupro/".$file_name;
        }
    }

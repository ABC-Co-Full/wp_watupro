...
function watupro_init() {	
   ...
   add_shortcode( 'watupro-myexams', 'watupro_myexams_code' );
	...
	if(version_compare($db_version, '5.64') == -1) watupro_activate(true);
...
function watupro_activate($update = false) {
   ...
   $collation = $wpdb->get_charset_collate();
      ...
      watupro_add_db_fields(array(      		
		 ...
		 watupro_add_db_fields(array(   		
      	...
      	array("name"=>"user_schedules", "type"=>"TEXT"), /* individual schedules per user, serialized array of arrays */
	               ...
	               // don't add new fields here - row size problems!
			 watupro_add_db_fields(array(
	    		array("name"=>"is_required", "type"=>"TINYINT UNSIGNED NOT NULL DEFAULT 0"),
	    		array("name"=>"correct_condition", "type"=>"VARCHAR(100) NOT NULL DEFAULT ''"),
	    		array("name"=>"max_selections", "type"=>"TINYINT UNSIGNED NOT NULL DEFAULT 0"),
				array("name"=>"min_selections", "type"=>"TINYINT UNSIGNED NOT NULL DEFAULT 0"),
	    		...
	    		array("name" => "open_end_mode", "type" => "VARCHAR(20) NOT NULL DEFAULT ''"),
	    		array("name" => "tags", "type" => "VARCHAR(191) NOT NULL DEFAULT ''"),
				array("name" => "open_end_display", "type" => "VARCHAR(191) NOT NULL DEFAULT 'medium'"),
		 	   ...
		 	   array("name"=>"title", "type"=>"VARCHAR(191) NOT NULL DEFAULT ''"),
			...
			watupro_add_db_fields(array(
				...
				array("name" => 'is_fallback', "type" => "TINYINT NOT NULL DEFAULT 0"), // only one grade could be the fallback grade for the quiz or globally
			), WATUPRO_GRADES);
      ...
      update_option("watupro_db_version", '5.64');
}
...
function watupro_vc_scripts($design_theme = null) {    
    ...
    wp_enqueue_style(
		...
		$translation_array = array('answering_required' => __('Answering this question is required', 'watupro'),
			...
			'min_sel_msg' => __('This question requires you to select a minimum of {{min-selections}} answers', 'watupro'),
			...
			'missed_required_question_num_min_sel' => __('Question number %s requires at least {{min-selections}} selected answers.', 'watupro'),
...
function watupro_add_menu_links() {
	...
	if(watupro_intel() and !current_user_can('manage_options')) {
		...
		$role_settings = get_option('watupro_role_settings');
	...
	else add_submenu_page('', sprintf(__('My %s', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD_PLURAL)), sprintf(__('My %s', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD_PLURAL)), $exam_caps, "my_watupro_exams", 'watupro_my_exams');
	...
	else add_submenu_page('', __("My Certificates", 'watupro'), __("My Certificates", 'watupro'), $exam_caps, 'watupro_my_certificates', 'watupro_my_certificates');
	...
	if(class_exists('WP_Experience_API')) {
	 ...
	 add_submenu_page('watupro_exams',__('Shortcode generator', 'watupro'), __('Shortcode generator', 'watupro'), $help_caps, "watupro_shortcode_generator", array('WatuPROShortcodeGen', 'generator'));
 	 ...
 	 // not visible in menu 
 	 add_submenu_page('',__('Add/Edit Exam', 'watupro'), __('Add/Edit Exam', 'watupro'), $exam_caps, "watupro_exam", "watupro_exam"); 
 	 add_submenu_page('',__('Add/Edit Question', 'watupro'), __('Add/Edit Question', 'watupro'), $exam_caps, "watupro_question", "watupro_question");  // add/edit question
 	 add_submenu_page('',__('Manage Questions', 'watupro'), __('Manage Questions', 'watupro'), $exam_caps, "watupro_questions", "watupro_questions");  // manage questions
 	 add_submenu_page('',__('Taken Exam Data', 'watupro'), __('Taken Exam Data', 'watupro'), $exam_caps, "watupro_takings", "watupro_takings");  // view takings
 	 add_submenu_page('',__('Manage Grades', 'watupro'), __('Manage Grades', 'watupro'), $exam_caps, "watupro_grades", "watupro_grades");  // manage grades
 	 add_submenu_page('',__('Copy Exam', 'watupro'), __('Copy Exam', 'watupro'), $exam_caps, "watupro_copy_exam", "watupro_copy_exam");  // copy exam
 	 add_submenu_page('',__('Users Who Earned Certificate', 'watupro'), __('Users Who Earned Certificate', 'watupro'), $certificate_caps, "watupro_user_certificates", "watupro_user_certificates");  // view/approve user certificates
 	  add_submenu_page('',__('Manually Award Certificate', 'watupro'), __('Manually Award Certificate', 'watupro'), $certificate_caps, "watupro_award_certificate", "watupro_manually_award_certificate");  // manually award certificates
 	 add_submenu_page('',__('Editing an answer to question', 'watupro'), __('Editing an answer to question', 'watupro'), $exam_caps, "watupro_edit_choice", "watupro_edit_choice"); 
 	 add_submenu_page('',__('Advanced questions import', 'watupro'), __('Advanced questions import', 'watupro'), $exam_caps, "watupro_advanced_import", "watupro_advanced_import"); 
 	 add_submenu_page('',__('User feedback on questions', 'watupro'), __('User feedback on questions', 'watupro'), $exam_caps, "watupro_questions_feedback", 'watupro_questions_feedback'); 
 	 add_submenu_page('',__('Assign users to a group', 'watupro'), __('Assign users to a group', 'watupro'), $exam_caps, "watupro_group_assign", 'watupro_group_assign'); 
 	 add_submenu_page('',__('Flashcard design settings', 'watupro'), __('Flashcard design settings', 'watupro'), $exam_caps, "watupro_flashcard_design", 
 	 	array('WatuPROFlashcard', 'design_settings'));
 	 add_submenu_page('',__('Custom design themes', 'watupro'), __('Custom design themes', 'watupro'), $exam_caps, "watupro_design_themes", 'watupro_design_themes');	
 	 add_submenu_page('',__('Schedules', 'watupro'), __('Schedules', 'watupro'), $exam_caps, "watupro_schedule", 'watupro_schedule');
 	 
 	 add_submenu_page('',__('Import Results', 'watupro'), __('Import Results', 'watupro'), $exam_caps, "watupro_import_takings", 'watupro_import_takings');
}

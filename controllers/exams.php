	...
	if(!empty($schedules) and is_array($schedules) and count($schedules)){
		foreach($schedules as $cnt => $schedule) {
			if($schedule['user_id'] == $user_id) {
   		   ...
   		   if(!watupro_check_schedule($schedule_from, $schedule_to, $quiz)) return false;
				break;
			}
...
function watupro_check_schedule($schedule_from, $schedule_to, $quiz = null) {
	...
	if ($now < $schedule_from or $now > $schedule_to) {
        ...
        $message = sprintf(__('This %1$s %2$s available between %3$s and %4$s.', 'watupro'), WATUPRO_QUIZ_WORD, $time_msg, date_i18n(get_option('date_format').' '.get_option('time_format'), $schedule_from), date_i18n(get_option('date_format').' '.get_option('time_format'), $schedule_to));

		// allow overriding this message
		$message = apply_filters('watupro_scheduled_quiz_not_available', $message, $schedule_from, $schedule_to, $quiz);
		echo $message;

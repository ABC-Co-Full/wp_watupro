...
WatuPRO.submitResult = function(e) { 
  ...
  jQuery("#action-email-button").show();	
...
WatuPRO.sendResult = function(e) { 

	// if name/email is asked for, it shouldn't be empty
	if(!this.validateEmailName()) return false;
	
	var data = {"action":'watupro_submit', "send_email": true, "quiz_id": this.exam_id, 'question_id[]': this.qArr,		
		"watupro_questions":  jQuery('#quiz-'+this.exam_id+' input[name=watupro_questions]').val(),
		"post_id" : this.post_id};		
	data = this.completeData(data);
	...
	if(!e || !e.no_ajax || e.no_ajax.value != 1) {
		try{
		    jQuery.ajax({ "type": 'POST', "url": this.siteURL, "data": data, "success": WatuPRO.successEmail, "error": WatuPRO.errHandle, "cache": false, dataType: "text"  });
		}catch(err){ alert(err)}
	}
}
...
WatuPRO.success = function(r) {  
   ...
   jQuery('#watupro_quiz .answer-results').html(r); 

   jQuery('#submittingExam'+WatuPRO.exam_id).hide();
...
WatuPRO.successEmail = function(r) {  
  jQuery('#watupro_quiz').html('<p>Sent successfully</p>'); 
return;
  // first check for recaptcha error, if yes, do not replace the HTML
  // but display the error in alert and return false;
  if(r.indexOf('WATUPRO_CAPTCHA:::')>-1) {
...
WatuPRO.showEndStep = function(skipAutoGenerated) {

...
class WTPQuestion {
	...
	static function prepare_vars(&$vars) {
      ...
      $vars['max_selections'] = intval($vars['max_selections'] ?? 0);
	  $vars['min_selections'] = intval($vars['min_selections'] ?? 0);
	  if($vars['min_selections'] > 0) $vars['is_required'] = 1;
	...
	static function add($vars) {
		...
		if(!empty($vars['column_width']) or !empty($vars['gaps_width'])) {
			$design = serialize(array(
				"column_width" => intval($vars['column_width'] ?? 0),
				"gaps_width" => intval($vars['gaps_width'] ?? 0),
			));
		} 
		
		// open end display - accept file?
		if(!empty($vars['accept_file_upload'])) {
            $vars['open_end_display'] .= '|file';
            if(!empty($vars['file_upload_hide_textarea'])) $vars['open_end_display'] .= '|hide-textarea';
        }
		...
		$sql = $wpdb->prepare("INSERT INTO ".WATUPRO_QUESTIONS." (exam_id, question, answer_type, 
			cat_id, explain_answer, is_required, sort_order, correct_condition, max_selections, min_selections, is_inactive, is_survey,
			...
			VALUES(%d, %s, %s, %d, %s, %d, %d, %s, %d, %d, %d, %d, %s, %s, %s, %s, %d, %s, %d, %d, %d, %d,
			...
			$vars['max_selections'], $vars['min_selections'], $vars['is_inactive'], $vars['is_survey'], $elaborate_explanation,
	...
	static function edit($vars, $id) {
      ...
      self :: prepare_vars($vars);
		...
		$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_QUESTIONS."
			...
			correct_condition=%s, max_selections=%d, min_selections=%d, is_inactive=%d, is_survey=%d, elaborate_explanation = %s,
			...
			$vars['is_required'],	$vars['correct_condition'], $vars['max_selections'], $vars['min_selections'],
  ...
  function display_choices($ques, $in_progress = null, $q_answers = null) {
     ...
     if($ques->is_flashcard and $exam->is_likert_survey) $ques->is_flashcard = 0;
      ...
      switch($ans_type) {
         ...
         $hide_textarea_css = $hide_textarea_class = '';
        
        if( strstr($ques->open_end_display, 'hide-textarea') ) {            
            $hide_textarea_css = ' style="display:none;" ';
            $hide_textarea_class = 'hide-textarea-'.$ques->ID;
        }
      	 
      	 // open_end_display may also contain "file" upload allowance like this: medium|file
        $allow_file_upload = $allow_audio = false;
        if(strstr($ques->open_end_display, '|')) {
            $parts = explode("|",   $ques->open_end_display);
            $ques->open_end_display = $parts[0];
            if(!empty($this->exam->no_ajax)) {
                $allow_file_upload = true;
                if(!empty($parts[2]) and $parts[2] == 'audio') $allow_audio = true;
            }
        }      
        
      	 switch($ques->open_end_display) {
      	 	 case 'text':      	 	 	
      	 	 	echo "<p $hide_textarea_css class='$hide_textarea_class'><input type='text' name='answer-{$ques->ID}[]' id='textarea_q_{$ques->ID}' class='watupro-text' value=\"$value\" size='60'></p>";
      	 	 break;	
      	 	 case 'medium':
      	 	 case 'large':
      	 	 default:
					 ...
					 if($ques->limit_words > 0) {
      	 	 	  ...
      	 	 	  echo "<p $hide_textarea_css class='$hide_textarea_class'><textarea name='answer-{$ques->ID}[]' id='textarea_q_{$ques->ID}' class='$class' rows='5' cols='80'>$value</textarea>\n";
      	 	 	  if($ques->limit_words > 0) {
			 ...
			 if($allow_file_upload) {
			    $audio_tag = $allow_audio ? ' accept="audio/*" capture="user" ' : '';
			 	$required_upload_class = empty($ques->file_upload_required) ? '' : 'watupro-file-upload-required-'.$ques->ID;
			 	echo "<p>".__('Upload file:', 'watupro')." <input type='file' name='file-answer-{$ques->ID}' $audio_tag class='watupro-file-upload $required_upload_class' id='watuproFileUpload".$ques->ID."-".$this->qct."'></p>";
			 }    	 
      	...
      	case 'checkbox':
      		...
      		foreach ($q_answers as $ans_cnt => $ans) {      		
      			if(!empty($exam->is_likert_survey)) {
	        		...
	        		// max selection limit?
	        		$maxsel_js = $maxsel_class = $minsel_class = '';
	        		...
	        		if($ques->answer_type == 'checkbox' and $ques->min_selections > 0) {
						$minsel_class = "watupro-min-selections-".$ques->min_selections." watupro-minsel-id-".$ques->ID;
					}
	        		...
	        		if($ques->open_end_display == 'dropdown') {
		        		...
		        		echo "<div class='watupro-question-choice $num_class $div_columns_class' dir='auto' $column_style>$enumerator_visible<input type='$ans_type' name='answer-{$ques->ID}[]' id='answer-id-{$ans->ID}' class='answer  $answer_class answerof-{$ques->ID} $maxsel_class $minsel_class' value='{$ans->ID}' $checked $maxsel_js $unselect_js/>";
    ...
    static function calc_answer($question, $answer, $choices = -1, &$user_grade_ids = null) {    	
		...
		if($question->answer_type == 'checkbox' or $question->answer_type == 'textarea') {			
			...
			if(empty($question->correct_condition) or $question->correct_condition == 'any') {
				...
				$precision = 8;
				// if the question is "calculate as a whole" selected we need to use max_points without this in mind because
				// the points here are calcuted before applying the cap
				$compare_max_points = $max_points;
				if($question->calculate_whole) {
					$question->calculate_whole = false;
					$old_max_allowed_points = $question->max_allowed_points;
					$question->max_allowed_points = 0;
					$compare_max_points = self :: max_points($question);
					$question->calculate_whole = true;
					$question->max_allowed_points = $old_max_allowed_points;
				}
				if(bccomp( number_format($points, $precision), number_format($compare_max_points, $precision), 3) >= 0) $is_correct = 1;
			}
    ...
    function display_hints($question, $in_progress = null) {
		...
		// already displayed all hints for this question?
        $qhints = explode("{{{split}}}", $question->hints);        
        if($num >= count($qhints))  $get_hints_link = false;

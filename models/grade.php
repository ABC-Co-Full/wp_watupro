...
class WTPGrade {
	...
	static function calculate($exam_id, $points, $percent, $cat_id = 0, $user_grade_ids = null, $pointspercent = 0) {
		...
		$grade_obj = (object)array("title"=>__('None', 'watupro'), "description"=>"", "ID" => 0);
		$do_redirect = false;
		$certificate_id = 0;
		...
		list($grade, $certificate_id, $do_redirect, $grade_obj) = apply_filters('watupro_calculate_grade', 
					array($grade, $certificate_id, $do_redirect, $grade_obj), $GLOBALS['watupro_taking_id'] ?? 0, $exam_id, $points, $percent, $cat_id, $user_grade_ids, $pointspercent);

		if($grade_obj->ID == 0) {
			$fallback = self :: replace_none_grade($grades);
			if(!empty($fallback->ID)) {
				$grade_obj = $fallback;

				[$grade, $grade_obj, $certificate_id, $do_redirect] = self :: match_grade($grade_obj);
			}
		}
	...
	static function calculate_dependent($exam_id, $catgrades_array, $achieved, $percent, $user_grade_ids = null, $pointspercent = 0, $certificate_id = 0) {
		...
		if($grade_obj->ID == 0) {
			...
			if(!empty($fallback->ID)) {
				$grade_obj = $fallback;
				[$grade, $grade_obj, $cert_id, $do_redirect] = self :: match_grade($grade_obj);
			}
		}
	...
	// allows a fallback grade to be applied if None is about to be assigned
	private static function replace_none_grade($grades) {
		$fallback = array_filter($grades, function($grade){
			if(!empty($grade->is_fallback)) return true;
			return false;
		});

		if(!empty($fallback) and count($fallback)) return reset($fallback);
	} // end replace_none_grade
}

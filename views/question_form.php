		...
		<div class="postbox" id="atdiv">
			...
			<div class="inside" style="padding:8px">
				...
				<div id="openEndText" class="wrap" style='display:<?php echo ($ans_type == 'textarea')?'block':'none'?>;'>
					...
					<?php if($exam->no_ajax):?>
						<p><input type="checkbox" name="accept_file_upload" value="1" <?php if(!empty($question->open_end_display) and strstr($question->open_end_display, '|file')) echo 'checked'?> onclick="if(this.checked && this.form.is_required.checked) { jQuery('#fileIsRequired').show() } else { jQuery('#fileIsRequired').hide()};if(this.checked) { jQuery('#hideTextarea').show() } else {jQuery('#hideTextarea').hide()};"> <?php _e('Accept also file upload with the answer', 'watupro')?></p>
					<?php endif;?>
				</div>
				...
				<span id="hideTextarea" style='display:<?php echo (empty($question->open_end_display) or !strstr($question->open_end_display, '|file') or !$exam->no_ajax) ? 'none' :'inline'?>'>&nbsp;&nbsp;<input type="checkbox" name="file_upload_hide_textarea" value="1" <?php if(strstr($question->open_end_display, '|hide-textarea')) echo "checked"?>> <?php _e("Hide text input", 'watupro');?></span>
				<?php endif;?></p>	
				...
				<div id="maxSelections" style='display:<?php echo (empty($question->ID) or $question->answer_type!='checkbox')?'none':'block'?>;'>
					<p><?php _e('Maximum selections allowed:','watupro')?> <input type="text" name="max_selections" value="<?php echo intval($question->max_selections??0)?>" size="4"> <?php _e('(Leave as 0 for unlimited)', 'watupro')?>
					<?php _e('Minimum selections allowed:','watupro')?> <input type="text" name="min_selections" value="<?php echo intval($question->min_selections??0)?>" size="4"> <?php _e('(Leave as 0 for unlimited)', 'watupro')?></p>

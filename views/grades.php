<div class="wrap watupro-wrap">
	...
	<?php if(count($cats) and (!watupro_intel() or empty($exam->is_personality_quiz))):?>
		<form method="post">
			<p><?php _e('Manage grades / results for category:', 'watupro')?> <select name="cat_id" onchange="this.form.submit();">
				<option value="0" <?php if($cat_id == 0) echo 'selected'?>><?php printf(__('- The Whole %s -', 'watupro'), __('Quiz', 'watupro'))?></option>
				<?php foreach($cats as $cat):?>
					<option value="<?php echo $cat->ID?>" <?php if($cat_id == $cat->ID) echo "selected"?>><?php echo ($cat->parent_id ? '&nbsp;&nbsp;&nbsp;-- ': '').stripslashes($cat->name)?></option>
				<?php endforeach;?>			
			</select></p>
   ...
   <?php endif;?>
	...
	<h2><?php if(empty($exam->is_personality_quiz)) _e('Add a New Grade', 'watupro');
	else _e('Add a New Personality Type', 'watupro');?></h2>
	...
	<form method="post" class="watupro-form" onsubmit="return validateGrade(this);">
		<div class="watupro-padded">
			...
			<?php endif;?>
			<p><input type="checkbox" name="is_fallback" value="1" class="is_fallback"> <?php _e('Use this is a fallback grade/result when no other is assigned.', 'watupro');?></p>
	...
	<?php foreach($grades as $grade):
		...
		<form method="post" class="watupro-form" onsubmit="return validateGrade(this);">
			...
			<?php endif;?>
				...
				<p><input type="checkbox" name="is_fallback" value="1" class="is_fallback" <?php if(!empty($grade->is_fallback)) echo 'checked'?>> <?php _e('Use this is a fallback grade/result when no other is assigned.', 'watupro');?></p>
...
// ensure only one fallback checkbox can be selected
document.addEventListener('DOMContentLoaded', function() {
	const checkboxes = document.querySelectorAll('.is_fallback');

...
<div class="wrap watupro-wrap">
	...
	<?php if(count($cats)):?>
		<table class="widefat watupro-table">
			<thead>		        
				<tr><th><?php _e('ID', 'watupro')?></th><th><?php _e('Category Name', 'watupro')?></th>
				<th><?php printf(__('Manage %s', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL)?></th>
				<th><?php printf(__('Shortcode for %s list', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL)?></th>
				<?php if(empty($parent->ID)):?>
		...
		<tbody>
			<?php foreach($cats as $cat):
				...
				<td><a href="admin.php?page=watupro_exams&cat_id=<?php echo $cat->ID?>&is_active=-1"><?php echo $cat->num_quizzes;?></a></td>
